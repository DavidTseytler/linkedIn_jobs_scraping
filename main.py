...
<head>
    ...
    <style>
        ...
        .jobs-table th, .jobs-table td { border: 1px solid #ddd; padding: 8px; text-align: left; }
        .jobs-table th { background-color: #f2f2f2; font-weight: bold; }
        .jobs-table tr:nth-child(even) { background-color: #f9f9f9; }
        .export-buttons { margin: 20px 0; }
        .platform-badge { padding: 2px 6px; border-radius: 3px; font-size: 12px; }
        .linkedin { background: #0077b5; color: white; }
        .indeed { background: #2164f4; color: white; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <form id="scrapeForm">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    ...
                    <input type="text" id="keywords" name="keywords" placeholder="software engineer" required>

                    <label for="location">Location:</label>
                    <input type="text" id="location" name="location" placeholder="United States">
                ...
                <div>
                    <label for="country_code">Country Code (for Indeed):</label>
                    <input type="text" id="country_code" name="country_code" placeholder="US" value="US">

                    <label for="max_jobs">Max Jobs:</label>
                    <input type="number" id="max_jobs" name="max_jobs" value="100" min="1" max="1000">

                    <label for="hours_old">Max Age (hours):</label>
                    <input type="number" id="hours_old" name="hours_old" placeholder="72" value="72">
                </div>
            </div>
    ...
    <script>
        document.getElementById('scrapeForm').addEventListener('submit', async function(e) {
            ...
            document.getElementById('results').innerHTML = `<p>Task created: ${result.id}. Status: ${result.status}</p>`;
        ...
        async function checkTaskStatus(taskId) {
            ...
            if (task.status === 'completed' || task.status === 'partial') {
                ...
                document.getElementById('results').innerHTML += `<p>Task failed</p>`;
            } else {
        ...
        async function loadJobs() {
            ...
            if (jobs.length > 0) {
                ...
                jobs.forEach(job => {
                    ...
                    row.innerHTML = `
                        <td><span class="platform-badge ${job.platform}">${job.platform}</span></td>
                        <td>${job.task_id}</td>
                        <td>${job.created_at}</td>
                        <td>${job.search_keywords}</td>
                        <td>${job.search_location}</td>
                        <td>${job.title}</td>
                        <td>${job.company}</td>
                        <td>${job.location}</td>
